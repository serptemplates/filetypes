...
export class DocumentationGenerator {
    registryManager;
    outputDir;
    constructor(registryManager, outputDir) {
     ...
     */
    async generateToolsDocumentation() {
     ...
     */
    async generateToolPage(tool, outputDir) {
        ...
        if (tool.content) {
            ...
            if (tool.content.aboutSection) {
                ...
                if (tool.content.aboutSection.fromFormat.details) {
                    md += `**Features:**\n`;
                    tool.content.aboutSection.fromFormat.details.forEach((detail) => {
                        md += `- ${detail}\n`;
                    });
                    md += `\n`;
                }
                ...
                if (tool.content.aboutSection.toFormat.details) {
                    md += `**Features:**\n`;
                    tool.content.aboutSection.toFormat.details.forEach((detail) => {
                        md += `- ${detail}\n`;
                    });
                    md += `\n`;
                }
            ...
            if (tool.content.relatedTools && tool.content.relatedTools.length > 0) {
                md += `## Related Tools\n\n`;
                tool.content.relatedTools.forEach((related) => {
     ...
     */
    async generateToolsIndex(tools, outputDir) {
 ...
 */
export function createDocumentationGenerator(registryManager, outputDir) {
