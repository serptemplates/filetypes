...
export class BatchToolImporter {
     ...
     */
    async analyzeImportRequests(requests) {
        ...
        for (const request of requests) {
            ...
            const exactMatch = existingTools.find(tool => tool.from === request.from &&
                tool.to === request.to &&
            ...
            const similarMatch = existingTools.find(tool => tool.from === request.from &&
                tool.to === request.to);
            if (similarMatch) {
                existing.push({
                    request,
     ...
     */
    findFuzzyMatch(request, existingTools) {
     ...
     */
    areFormatsEquivalent(format1, format2) {
        if (format1 === format2)
            return true;
     ...
     */
    async executeImport(requests, options = {}) {
        const created = [];
        const skipped = [];
        const errors = [];
        for (const request of requests) {
     ...
     */
    generateImportReport(analysis, execution) {
    ...
    determineOperation(from, to) {
        ...
        if (imageFormats.has(from) && imageFormats.has(to))
            return 'convert';
        if (videoFormats.has(from) && videoFormats.has(to))
            return 'convert';
        if (audioFormats.has(from) && audioFormats.has(to))
            return 'convert';
        if (videoFormats.has(from) && imageFormats.has(to))
            return 'convert'; // video thumbnail
        if (videoFormats.has(from) && audioFormats.has(to))
            return 'extract'; // audio extraction
        return 'convert'; // default
    }
    /**
     * Generate a Tool object from an import request
     */
    generateToolFromRequest(request) {
 ...
 */
export function createBatchToolImporter(registryManager) {
    return new BatchToolImporter(registryManager);
}
//# sourceMappingURL=batch-importer.js.map