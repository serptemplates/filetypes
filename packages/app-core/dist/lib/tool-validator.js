...
export class ToolValidator {
    ...
    setupDefaultValidators() {
        ...
        this.customValidators.set('tool-structure', async (tool) => {
        ...
        this.customValidators.set('route-format', async (tool) => {
            const start = Date.now();
        ...
        this.customValidators.set('content-completeness', async (tool) => {
            const start = Date.now();
            if (!tool.content) {
                return {
        ...
        this.customValidators.set('faq-quality', async (tool) => {
            const start = Date.now();
            ...
            const metrics = {
                ...
                averageQuestionLength: faqs.reduce((sum, faq) => sum + faq.question.length, 0) / faqs.length,
                averageAnswerLength: faqs.reduce((sum, faq) => sum + faq.answer.length, 0) / faqs.length,
                hasVariedQuestions: new Set(faqs.map((faq) => faq.question.toLowerCase())).size === faqs.length
            };
            if (metrics.hasMinimumCount)
                qualityScore += 25;
            if (metrics.averageQuestionLength >= 10 && metrics.averageQuestionLength <= 100)
                qualityScore += 25;
            if (metrics.averageAnswerLength >= 50)
                qualityScore += 25;
            if (metrics.hasVariedQuestions)
                qualityScore += 25;
        ...
        this.customValidators.set('format-compatibility', async (tool) => {
            const start = Date.now();
            ...
            const fromGroup = Object.keys(formatGroups).find(group => formatGroups[group].includes(tool.from));
            const toGroup = Object.keys(formatGroups).find(group => formatGroups[group].includes(tool.to));
        ...
        this.customValidators.set('security-check', async (tool) => {
            const start = Date.now();
     ...
     */
    async validateTool(tool, testIds) {
