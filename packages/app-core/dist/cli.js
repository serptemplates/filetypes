...
import { createToolGenerator } from './lib/tool-generator.js';
import { createRegistryManager } from './lib/tool-registry.js';
import { createBatchToolImporter } from './lib/batch-importer.js';
...
program
    ...
    try {
        ...
        // Calculate stats from registry tools
        const stats = {
        ...
        allTools.forEach(tool => {
            stats.byOperation[tool.operation] = (stats.byOperation[tool.operation] || 0) + 1;
            if (tool.from) {
                stats.byFormat[tool.from.toLowerCase()] = (stats.byFormat[tool.from.toLowerCase()] || 0) + 1;
            }
            if (tool.to) {
                stats.byFormat[tool.to.toLowerCase()] = (stats.byFormat[tool.to.toLowerCase()] || 0) + 1;
            }
        });
...
program
    .command('sync')
    .description('Sync registry to tools.json file')
    .option('--to-registry', 'Sync from tools.json to registry (reverse)')
    .action(async (options) => {
    try {
        const registryManager = createRegistryManager(DEFAULT_REGISTRY_PATH);
        ...
        if (options.toRegistry) {
            // Sync from tools.json to registry
            console.log(chalk.blue('🔄 Syncing tools.json → registry...'));
            ...
            // Default: Sync from registry to tools.json
            console.log(chalk.blue('🔄 Syncing registry → tools.json...'));
            const allTools = await registryManager.getAllTools();
            await fs.writeFile(toolsPath, JSON.stringify(allTools, null, 2), 'utf-8');
            console.log(chalk.green(`✅ Synced ${allTools.length} tools to ${toolsPath}`));
        }
    }
    catch (error) {
