...
import toolsData from '@serp-filetypes/app-core/data/tools.json';
...
export function transformFileTypeData(raw: FileTypeRawData): FileTypeTemplateData {
  ...
  const whatIs = raw.summary ||
    ...
    `A .${raw.extension} file is a ${raw.name}${(raw as any).developer_name ? ` developed by ${(raw as any).developer_name}` : ''}.`;
  ...
  return {
    ...
    developer: (raw as any).developer || (raw as any).developer_name || (raw as any).developer_org,
    image: {
    ...
    lastUpdated: raw.last_updated || (raw as any).updated_at || new Date().toISOString()
  };
}
